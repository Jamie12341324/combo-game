// from stackoverflow
/*jslint browser:true */
// In this file there are no comments to show which parts
// are copied from tutorials because none of the code
// is from a tutorial however I did need
// some help from copilot with commands relating
// to intervals, timeouts, clearing them and some scope things
// global variables for doing things to do with timers
// that can be cleared from anywhere in the code
// all functions first written in this file so not ones
// that are built into javascript like setInterval() were written by me
// function for updating the length of arrays when L/L2
// increases as to not cause errors when trying
// to clear timers from anywhere in the code
...
document.addEventListener("DOMContentLoaded", function(){
    ...
    button.addEventListener("click", function(){
        ...
        if (num1>0 || num2>0){
            ...
            let money=parseInt(moneystr.substring(6,Lm));
            ...
            if (money!==money1){
                ...
                let paraOld=allyDiv1.children[allyDiv1.children.length-5];
                ...
                let reference=allyDiv1.children[allyDiv1.children.length-4];
                // insertBefore from copilot
    ...
    // Adds a EventListener to a button that toggles
    // the screen to a enemy background image
    // ends the flipping between bold and regular if
    // it is happening and sets the text to regular
    ...
    // Adds a EventListener to a button that starts
    // a battle between the boss and allied combos
    // where the combos are shown to be fighting by
    // swaping between bold and regular text
    // it also disables the button while the battle
    ...
    // Adds a EventListener to a button that
    // toggles the screen to a boss 1 background image
    // ends the flipping between bold and
    // regular if it is happening and sets the text to regular
    ...
    // Adds a EventListener to a button
    // that starts a battle between the boss and allied combos
    // where the combos are shown to be
    // fighting by swaping between bold and regular text
    // it also disables the button while the
    ...
    // Adds a EventListener to a button that
    // toggles the screen to a boss 2 background image
    // ends the flipping between bold and
    // regular if it is happening and sets the text to regular
    ...
    // Adds a EventListener to a button that
    // starts a battle between the boss and allied combos
    // where the combos are shown to be
    // fighting by swaping between bold and regular text
    // it also disables the button while the
    // battle is taking place so the system does not glitch
    ...
    homeButton.addEventListener("click", function(){
        let exampleId="back1";
        addOnlyToggle(instructions);
        addOnlyToggle(summoning);
        toggle2(main,exampleId);
        removeOnlyToggle(enemies);
        removeOnlyToggle(boss1);
        removeOnlyToggle(boss2);
    });
    // setInterval from copilot
    let checker=setInterval(checkNum,1000);
});
...
// toggle2 changes a id to back1 if it is
// already a certain id and changes it to that id if it is not
function toggle2(main,exampleId){
    if (main[0].id===exampleId){
        // help from copilot with overwriting id
        main[0].id="back1";
    }else{
        // help from copilot with overwriting id
        main[0].id=exampleId;
...
// addOnlyToggle shows a element if it is
// hidden and leaves it alone if it is not
function addOnlyToggle(div){
    if (div.style.display==="none"){
        div.style.display="block";
    }
}
// makeBold adds a class to make text bold
// if that class is not already there and removes it if it is
function makeBold(p,r){
    if (p.classList.contains("battleBold")===false){
        // help from copliot making text bold on and off
...
// checkNum gets numbers from input boxs
// and updates the cost of the selection
function checkNum(){
    let num1=parseInt(document.getElementById("answer-box").value);
    let num2=parseInt(document.getElementById("answer-box2").value);
    let cost=num1+num2;
...
// comboMaker makes a sequence of letters
// with num1 as the length of the sequence
...
// exchange works out if you can buy something
// and passes back the new amount of money you
// have after you buy something
function exchange(credits,cost){
    let remainder=credits-cost;
    if (remainder<0){
        window.alert("You don't have the "
            +"credits for that");
        return credits;
    }
    return remainder;
}
// allyPower uses classes for the allied
// combos to get an array of numbers for
// the power of all the combos
function allyPower(){
    let c3=0;
    let L3=document.getElementById("allyDiv").children.length-5;
    let allyList;
    allyList=new Array(L3);
    while (c3<L3){
        let i=c3+1;
...
// enemyPower uses classes for the enemy
// combos to get an array of numbers for the power of all the combos
function enemyPower(){
    let c3=0;
    let L3=document.getElementById("enemies").children.length-2;
    let enemyList;
    enemyList=new Array(L3);
    while (c3<L3){
        let i=c3+1;
...
function battleSim(foeId,attackButton){
    ...
    let p;
    let p2;
    // this while goes through allies and enemies
    // in order and decides the winner and takes the
    // losers power off the winners and
    // repeates the process with the next ally or
    // enemy depending on who was the winner
    while (c<L && c2<L2){
        if (allyList[c]<=enemyList[c2]){
            enemyList[c2]=enemyList[c2]-allyList[c];
            c=c+1;
            let i2=c2+1;
            // flashTimeStart make enemies and allies
            // flash to show there fighting
            setTimeout(() => {
            ...
            // flashTimeEnd gets rid of flashing after
            // enemies and allies are finsished fighting
            // setTimeout from copilot
            setTimeout(() => {
            if (allyBold[c10A]){
                // clearInterval from copilot
    ...
    // has won or lost the battle, tells the player
    // was extra money they got if
    // they won, gives them the money and adds a enemy
    // or makes the boss more powerful
    setTimeout(() => {
        if (c===L){
        window.alert("Battle Lost");
        }
        if (c2===L2){
            window.alert("Battle Victory");
            if (foeId==="enemy"){
                window.alert("You earned 5 money for defeating the enemies");
                addMoney(5);
                addEnemy();
            }
            if (foeId==="boss1div"){
                window.alert("You earned 8 money for defeating the boss");
                addMoney(8);
                upgradeBoss(foeId);
            }
            if (foeId==="boss2div"){
                window.alert("You earned 15 money for defeating the boss");
                addMoney(15);
                upgradeBoss(foeId);
            }
        }
...
// returns the next enemy to be processed by
// battleSim depending on the foeId which changes
// depending on which button was clicked to
// trigger battleSim
function getEnemy(i,foeId){
    let p2;
    if (foeId==="enemy"){
        p2=document.getElementById("enemy"+i);
    }else if (foeId==="boss1div"){
        p2=document.getElementById("boss1div").children[1];
    }else if (foeId==="boss2div"){
        p2=document.getElementById("boss2div").children[1];
    }
    return p2;
}
// toggleButtons disables all the buttons
// while a attack is happeing to avoid glitches
// and can be used to toggle them back
// after an attack has finsished
function toggleButtons(){
    // help from copliot disabling buttons
...
// addEnemy creates a p element with an id
// with enemy then a number which is one more
// than the last enemies number it also gives
// a class of ally-text-color to the element
// so it is readable but also a another class
...
function addEnemy(){
    ...
    para.classList.add(power);
    para.innerText=combo;
    let reference=enemyDiv.children[enemyDiv.children.length-1];
    enemyDiv.insertBefore(para,reference);
}
// upgradeBoss creates a longer combo than
// the orginal combo for the boss and
...
function upgradeBoss(foeId){
    ...
    let Lp=p.innerText.length+1;
    let num1=Lp;
    let num2=Lp;
    let combo=comboMaker(num1,num2);
    let num3=p.classList[1].substring(5,p.classList[1].length);
    let orginalPower=parseInt(num3);
